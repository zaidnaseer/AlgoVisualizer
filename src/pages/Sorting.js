...
const markSortedPrefix = (colors, endIdx) => { for (let k = 0; k <= endIdx; k++) colors[k] = '#4ade80'; };

async function selectionScanForMin(a, i, n, colors, setColorState, sleepFn, stopRef, counts, setStats) {
  let minIdx = i;
  for (let j = i + 1; j < n; j++) {
...
async function selectionSwapIfNeeded(a, i, minIdx, setArrayState, sleepFn, counts) {
  if (minIdx === i) return;
...
const Sorting = () => {
  ...
  useEffect(() => {
    const randomArray = Array.from({ length: arraySize }, () => Math.floor(Math.random() * 300) + 10);
  ...
  const bubbleSortWithStop = async (arr, setArrayState, setColorState, sleepFn, stopRef, setStats) => {
    const a = [...arr];
    const n = a.length;
  ...
  const selectionSortWithStop = async (arr, setArrayState, setColorState, sleepFn, stopRef, setStats) => {
    const a = [...arr];
    const n = a.length;
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      colors[i] = '#ffd93d';
      setColorState([...colors]);
      const minIdx = await selectionScanForMin(a, i, n, colors, setColorState, sleepFn, stopRef, counts, setStats);
  ...
  const insertionSortWithStop = async (arr, setArrayState, setColorState, sleepFn, stopRef, setStats) => {
    const a = [...arr];
    const n = a.length;
    let comparisons = 0, swaps = 0;
    for (let i = 1; i < n; i++) {
  ...
  const mergeSortWithStop = async (arr, setArrayState, setColorState, sleepFn, stopRef, setStats) => {
    const a = [...arr];
    let comparisons = 0, swaps = 0;
  ...
  const quickSortWithStop = async (arr, setArrayState, setColorState, sleepFn, stopRef, setStats) => {
    const a = [...arr];
    const n = a.length;
    let comparisons = 0, swaps = 0;
  ...
  const runRadixSort = async () => {
    ...
    return radixSort(array, setArray, setColorArray, delayRef.current, stopSortingRef, (s) => setStatistics(prev => ({ ...prev, ...s })));
  };
  const runBucketSort = async () => {
    const { bucketSort } = await import('../algorithms/bucketSort');
    return bucketSort(array, setArray, setColorArray, delayRef.current, stopSortingRef, (s) => setStatistics(prev => ({ ...prev, ...s })));
  };
  ...
  const handleSort = async () => {
    ...
    try {
      switch (algorithm) {
        case 'bubbleSort':
          await bubbleSortWithStop(array, setArray, setColorArray, sleep, stopSortingRef, (s) => setStatistics(prev => ({ ...prev, ...s })));
          break;
        case 'selectionSort':
          await selectionSortWithStop(array, setArray, setColorArray, sleep, stopSortingRef, (s) => setStatistics(prev => ({ ...prev, ...s })));
          break;
        case 'insertionSort':
          await insertionSortWithStop(array, setArray, setColorArray, sleep, stopSortingRef, (s) => setStatistics(prev => ({ ...prev, ...s })));
          break;
        case 'mergeSort':
          await mergeSortWithStop(array, setArray, setColorArray, sleep, stopSortingRef, (s) => setStatistics(prev => ({ ...prev, ...s })));
          break;
        case 'quickSort':
          await quickSortWithStop(array, setArray, setColorArray, sleep, stopSortingRef, (s) => setStatistics(prev => ({ ...prev, ...s })));
          break;
        case 'radixSort':
          await runRadixSort();
          break;
  ...
  const algoOptions = ['bubbleSort', 'selectionSort', 'insertionSort', 'mergeSort', 'quickSort', 'radixSort', 'bucketSort'];

  return (
    <div className="page-container" style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      <h1 className="page-title" style={{ textAlign: 'center', marginBottom: '30px' }}>Sorting Algorithms</h1>
      ...
      <div className="controls-section" style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap', justifyContent: 'center', marginBottom: '16px' }}>
        ...
        <button className="btn btn-secondary" onClick={handleStop} disabled={!isSorting}>Stop</button>
        <button className="btn btn-secondary" onClick={generateArray} disabled={isSorting}>Generate Array</button>
      </div>
        ...
        <div style={{ background: 'rgba(15, 52, 96, 0.1)', borderRadius: '15px', border: '1px solid rgba(102,204,255,0.2)', padding: '20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'space-between', marginBottom: '14px' }}>
            ...
            <input
              ...
              style={{ width: '200px' }}
            />
            <div style={{ color: '#66ccff', fontWeight: 600, minWidth: '140px', textAlign: 'right' }}>{arraySize} <span style={{ color: '#9bb3c7', fontWeight: 400 }}>(elements)</span></div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'space-between' }}>
            ...
            <input
              ...
              style={{ width: '200px' }}
            />
            <div style={{ color: '#66ccff', fontWeight: 600, minWidth: '140px', textAlign: 'right' }}>{delay}ms</div>
          </div>
        </div>
  <SimpleExportControls containerId="sort-visualization-container" />
      ...
      <div style={{ display: 'flex', flexDirection: isTabletOrBelow ? 'column' : 'row', flexWrap: 'wrap', gap: '30px', alignItems: 'flex-start', marginBottom: '30px' }}>
        <div style={{ flex: '1 1 auto', minWidth: '300px', maxWidth: '100%', overflowX: 'hidden' }}>
          <div id="sort-visualization-container" className="visualization-area" style={{ minHeight: '400px', padding: '20px', background: 'rgba(15, 52, 96, 0.1)', borderRadius: '15px', border: '1px solid rgba(102, 204, 255, 0.2)', margin: '20px 0' }}>
        ...
        <div style={{ flex: '0 0 300px', minWidth: '280px', maxWidth: '100%', background: 'rgba(102,204,255,0.07)', border: '1px solid rgba(102,204,255,0.15)', borderRadius: '12px', padding: '18px', overflowX: 'auto' }}>
          ...
          <div style={{ background: 'rgba(102,204,255,0.08)', borderRadius: '8px', padding: '10px 12px', fontSize: '14px', color: '#b8c5d1', minHeight: '40px' }}>
            ...
            <br />
            {(ALGORITHM_PSEUDOCODE[algorithm] || [])[0]?.explain || 'Select an algorithm to view its pseudocode.'}
          </div>
        </div>
      </div>
          ...
          return (
            <div>
              ...
              <div className="complexity-grid">
                <div className="complexity-row"><span className="complexity-label">Time:</span> <span style={{ color: '#e0e6ed' }}>{meta.timeComplexity}</span></div>
                <div className="complexity-row"><span className="complexity-label">Space:</span> <span style={{ color: '#e0e6ed' }}>{meta.spaceComplexity}</span></div>
                <div className="complexity-row"><span className="complexity-label">Best Case:</span> <span style={{ color: '#e0e6ed' }}>{meta.bestCase}</span></div>
                <div className="complexity-row"><span className="complexity-label">Stable:</span> <span style={{ color: '#e0e6ed' }}>{meta.stable}</span></div>
              </div>
            </div>
          );
        })()}
