...
import { LinkedList, getLinkedListSteps, LINKED_LIST_PSEUDOCODE } from '../algorithms/linkedList';
...
import { BinaryTree, getTreeSteps, BINARY_TREE_PSEUDOCODE } from '../algorithms/binaryTree';
...
function DataStructures() {
    ...
    const dataStructures = {
        ...
        avl: {
            ...
            timeComplexity: { insertion: 'O(log n)', deletion: 'O(log n)', search: 'O(log n)' },
            spaceComplexity: 'O(n)',
            useCases: ['Ordered data indexing', 'Databases and memory-resident indexes']
        },
        redblack: {
            title: 'Red-Black Tree',
            description: 'A balanced binary search tree with an extra color bit per node that ensures logarithmic height.',
            timeComplexity: { insertion: 'O(log n)', deletion: 'O(log n)', search: 'O(log n)' },
            spaceComplexity: 'O(n)',
        ...
        graph: {
            title: 'Graph',
            description: 'A collection of nodes (vertices) connected by edges; supports many algorithms like BFS/DFS, shortest paths.',
            timeComplexity: { traversal: 'O(V + E)' },
            spaceComplexity: 'O(V + E)',
            useCases: ['Navigation and routing', 'Social networks', 'Dependency analysis']
        }
    };
    ...
    useEffect(() => {
        ...
        const seedStack = () => {
            if (stackRef.size === 0) {
                ...
                stackRef.top = null; stackRef.size = 0; [1, 2, 3, 4].forEach((v) => stackRef.push(v));
            }
        };
        const seedQueue = () => {
            if (queueRef.size === 0) {
        ...
        seedLinkedList(); seedStack(); seedQueue(); seedTree();
    ...
    const comingSoon = selectedStructure === 'avl' || selectedStructure === 'redblack' || selectedStructure === 'graph';
    ...
    return (
        <div className="page-container">
            ...
            <div className="controls-section" style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '8px' }}>
                <label className="label" htmlFor="dsSelect">Select Data Structure:</label>
                <select
                    className="select"
                    id="dsSelect"
                    value={selectedStructure}
                    onChange={(e) => { setSelectedStructure(e.target.value); setSteps([]); setCurrentStep(0); setMessage(''); }}
                >
            ...
            <div className="controls-section" style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '8px' }}>
                {comingSoon ? (
                    <div style={{ color: '#b8c5d1', fontWeight: 600 }}>This data structure is coming soon. Visualizations and operations are not yet available.</div>
                ) : (
                    <>
                        <input className="input" type="number" placeholder="Value" value={inputValue} onChange={(e) => setInputValue(e.target.value)} />
                        {selectedStructure === 'linkedlist' && (
                            <input className="input" type="number" placeholder="Index (optional)" value={indexValue} onChange={(e) => setIndexValue(e.target.value)} />
                        )}
                        <select id="operationSelect" className="select" value={operation} onChange={(e) => setOperation(e.target.value)}>
                            {selectedStructure === 'linkedlist' && (
                                <>
                        ...
                        <button className="btn btn-secondary" onClick={() => { setSteps([]); setCurrentStep(0); setMessage(''); setInputValue(''); setIndexValue(''); }}>Reset</button>
                    </>
                )}
            </div>

            {/* 3) Visualization Controls (size only) + Export */}
            <div className="controls-section" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '12px' }}>
                <div style={{ background: 'rgba(15, 52, 96, 0.1)', borderRadius: '15px', border: '1px solid rgba(102,204,255,0.2)', padding: '20px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'space-between' }}>
                        <label className="label" htmlFor="sizeRange" style={{ minWidth: '110px' }}>Node Size:</label>
                        <input
                            id="sizeRange"
                            type="range"
                            min="40"
            ...
            {!comingSoon && steps.length > 0 && (
                <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                    <button className="btn btn-secondary" onClick={() => setCurrentStep((s) => Math.max(0, s - 1))} disabled={currentStep === 0}>Previous Step</button>
                    <button className="btn btn-secondary" onClick={() => setCurrentStep((s) => Math.min(steps.length - 1, s + 1))} disabled={currentStep >= steps.length - 1}>Next Step</button>
                    <span style={{ color: '#66ccff', fontWeight: 600 }}>Step {currentStep + 1} / {steps.length}</span>
                </div>
            )}

            {/* 4) Visualization & Pseudocode (two-column like Searching) */}
            <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: '30px', alignItems: 'flex-start', marginBottom: '30px' }}>
                <div style={{ flex: '1 1 auto', minWidth: '300px', maxWidth: '100%', overflowX: 'hidden' }}>
                    ...
                    >
                        {comingSoon && (
                            <div style={{ textAlign: 'center', color: '#b8c5d1', fontWeight: 600, fontSize: 18 }}>
                                {currentStructure?.title || 'This'} visualization is coming soon.
                            </div>
                        )}
                        {selectedStructure === 'linkedlist' && (
                            (() => {
                                ...
                                return (
                                    <LinkedListHeaderBadge head={head} tail={tail} />
                                );
                            })()
                        )}
                        {!comingSoon && selectedStructure === 'linkedlist' && (
                            <LinkedListViz state={steps[currentStep]} list={listRef} vizSize={vizSize} />
                        )}
                        {!comingSoon && selectedStructure === 'stack' && (
                            <StackViz state={steps[currentStep]} stack={stackRef} vizSize={vizSize} />
                        )}
                        {!comingSoon && selectedStructure === 'queue' && (
                            <QueueViz state={steps[currentStep]} queue={queueRef} vizSize={vizSize} />
                        )}
                        {!comingSoon && selectedStructure === 'tree' && (
                            <TreeViz state={steps[currentStep]} tree={treeRef} vizSize={vizSize} />
                        )}
                    </div>
                </div>
                {!comingSoon && (
                    <div style={{ flex: '0 0 420px', minWidth: '280px' }}>
                        <PseudocodePanel
                            ...
                            map={{ linkedlist: LINKED_LIST_PSEUDOCODE, stack: STACK_PSEUDOCODE, queue: QUEUE_PSEUDOCODE, tree: BINARY_TREE_PSEUDOCODE }[selectedStructure]}
                        />
                    </div>
                )}
            </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px', marginBottom: '12px' }}>
                        <div style={{ background: 'rgba(102, 204, 255, 0.08)', padding: '12px', borderRadius: '10px', border: '1px solid rgba(102, 204, 255, 0.2)' }}>
                            ...
                            <div style={{ color: '#e0e6ed' }}>
                                {Object.entries(currentStructure.timeComplexity).map(([op, complexity]) => (
                                    <div key={op}>{op}: {complexity}</div>
                                ))}
                            </div>
                        </div>
                        <div style={{ background: 'rgba(102, 204, 255, 0.08)', padding: '12px', borderRadius: '10px', border: '1px solid rgba(102, 204, 255, 0.2)' }}>
                            <div style={{ color: '#66ccff', fontWeight: 600, marginBottom: '6px' }}>Space Complexity</div>
                            <div style={{ color: '#e0e6ed' }}>{currentStructure.spaceComplexity}</div>
                        </div>
                    </div>
                    <div>
                        <div style={{ color: '#66ccff', fontWeight: 600, marginBottom: '6px' }}>Common Use Cases</div>
                        ...
                        {comingSoon && (
                            <div style={{ marginTop: 12, color: '#b8c5d1', fontWeight: 600 }}>Detailed visualization and operations are coming soon.</div>
                        )}
                    </div>
                </div>
            </div>
...
function Badge({ children, active, shape = 'square', size = 48, animate = false }) {
    const { theme } = useTheme();
    const activeBg = theme === 'light' ? 'rgba(46, 204, 113, 0.18)' : 'rgba(46, 204, 113, 0.28)'; // green, clearer in light mode
    const activeBorder = theme === 'light' ? 'rgba(46, 204, 113, 0.70)' : 'rgba(46, 204, 113, 0.80)';
    const activeShadow = theme === 'light' ? '0 0 0 2px rgba(46, 204, 113, 0.25) inset' : '0 0 0 2px rgba(46, 204, 113, 0.35) inset';
    ...
    const base = {
        ...
        border: active ? `1px solid ${activeBorder}` : '1px solid rgba(102,204,255,0.3)',
    color: textColor,
        textAlign: 'center',
        fontWeight: 700,
...
function LinkedListViz({ state, list: live, vizSize = 56 }) {
    ...
    return (
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap', padding: 20, minHeight: 120 }}>
            {list.map((v, i) => (
                <React.Fragment key={`${v}-${i}`}>
            <Badge active={i === hi} animate={i === hi} shape="circle" size={vizSize}>{v}</Badge>
                    {i < list.length - 1 && <span style={{ color: '#66ccff', fontSize: 22 }}>→</span>}
                </React.Fragment>
            ))}
            {/* list always seeded; no empty message */}
            {state?.description && <div style={{ width: '100%', textAlign: 'center', color: '#66ccff', marginTop: 8 }}>{state.description}</div>}
        </div>
    );
}
...
function LinkedListHeaderBadge({ head, tail }) {
    const { theme } = useTheme();
    const border = theme === 'light' ? '1px solid rgba(46, 204, 113, 0.6)' : '1px solid rgba(102,204,255,0.35)';
    const bg = theme === 'light' ? 'rgba(46, 204, 113, 0.12)' : 'rgba(15, 52, 96, 0.25)';
    const color = theme === 'light' ? '#1a1a1a' : '#e0e6ed';
    return (
        <div style={{ position: 'absolute', right: 16, top: 16 }}>
            <div style={{ display: 'inline-flex', gap: 10, alignItems: 'center', padding: '6px 12px', borderRadius: 14, border, background: bg, color, fontWeight: 700 }}>
                <span>Head: <span style={{ opacity: 0.9 }}>{head}</span></span>
                <span style={{ opacity: 0.5 }}>•</span>
                <span>Tail: <span style={{ opacity: 0.9 }}>{tail}</span></span>
            </div>
        </div>
    );
}
LinkedListHeaderBadge.propTypes = { head: PropTypes.oneOfType([PropTypes.string, PropTypes.number]), tail: PropTypes.oneOfType([PropTypes.string, PropTypes.number]) };

function StackViz({ state, stack, vizSize = 56 }) {
    const arr = state?.stack ?? stack.getArray();
    const hi = state?.highlightIndex ?? -1;
    return (
    <div style={{ display: 'flex', gap: 30, alignItems: 'flex-end', justifyContent: 'center', padding: 20 }}>
            {/* seeded; no empty message */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {arr.map((v, i) => (
                    <Badge active={i === hi} animate={i === hi} key={`${v}-${i}`} shape="square" size={vizSize}>{v}</Badge>
                ))}
            </div>
            {state?.description && <div style={{ color: '#66ccff' }}>{state.description}</div>}
        </div>
    );
}
...
function TreeViz({ state, tree, vizSize = 56 }) {
    ...
    while (q.length) {
        ...
        if (node) {
            q.push({ node: node.left, path: `${path}0`, depth: depth + 1, pos: pos * 2 });
            q.push({ node: node.right, path: `${path}1`, depth: depth + 1, pos: pos * 2 + 1 });
        }
    }
    const filtered = levels.filter((lvl) => (lvl?.some((n) => n.data !== null)) ?? false);
    const width = 640; const levelHeight = 100; const height = Math.max(140, filtered.length * levelHeight + 40);
    const nodes = [];
    const edges = [];
    ...
    nodes.forEach((n) => {
        ...
        if (left && left.data !== null && n.data !== null) edges.push({ x1: n.x, y1: n.y, x2: left.x, y2: left.y });
        if (right && right.data !== null && n.data !== null) edges.push({ x1: n.x, y1: n.y, x2: right.x, y2: right.y });
    });
    return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 20, width: '100%' }}>
            {/* seeded; no empty state */}
            {filtered.length > 0 && (
                <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', maxWidth: width, height }}>
                    {/* edges */}
                    {edges.map((e) => (
                        <line key={`e-${e.x1}-${e.y1}-${e.x2}-${e.y2}`} x1={e.x1} y1={e.y1} x2={e.x2} y2={e.y2} stroke="#3d7bd9" strokeWidth="3" />
                    ))}
                    {/* nodes */}
                    {nodes.filter((n) => n.data !== null).map((n) => {
                        ...
                        if (isActive) {
                            fill = theme === 'light' ? 'rgba(46, 204, 113, 0.18)' : 'rgba(46, 204, 113, 0.28)';
                        }
                        return (
                            <g key={n.path}>
                                <circle cx={n.x} cy={n.y} r={r} fill={fill} stroke={stroke} strokeWidth="2" style={{ transition: 'r 200ms ease, stroke 200ms ease, fill 200ms ease' }} />
                                <text x={n.x} y={n.y + 7} textAnchor="middle" fontWeight="700" fontSize={Math.max(14, vizSize/3)} fill={theme === 'light' ? '#1a1a1a' : '#e0e6ed'}>{n.data}</text>
                            </g>
                        );
                    })}
                </svg>
...
function PseudocodePanel({ setKey, state, map }) {
    ...
    keyCandidate = keyCandidate || state?.operation || state?.op || inferPseudoKey(setKey, state);
    let fallback = 'search';
    if (setKey === 'linkedlist') fallback = 'traverse';
    else if (setKey === 'stack') fallback = 'push';
    else if (setKey === 'queue') fallback = 'enqueue';
    const seq = (map && keyCandidate && map[keyCandidate]) ? map[keyCandidate] : (map?.[fallback] || []);
    const lineIdx = state?.pseudoLine ?? null;
    return (
        <div style={{ background: 'rgba(102,204,255,0.07)', border: '1px solid rgba(102,204,255,0.15)', borderRadius: 12, padding: 14, marginTop: 12, width: '100%', maxWidth: 560 }}>
            <h4 style={{ color: '#66ccff', marginBottom: 8 }}>Pseudocode</h4>
            <pre style={{ background: 'rgba(26,26,46,0.95)', padding: 12, borderRadius: 8, color: '#e0e6ed', overflowX: 'auto' }}>
                {seq.map((l, idx) => (
                    <div key={l.code} style={{
                        background: lineIdx !== null && idx === lineIdx ? 'rgba(102,204,255,0.25)' : 'none',
                        color: lineIdx !== null && idx === lineIdx ? '#66ccff' : '#e0e6ed',
                        borderRadius: 6,
                        padding: '2px 6px',
            ...
            {state?.description && <div style={{ color: '#b8c5d1' }}>{state.description}</div>}
        </div>
    );
}
...
function inferPseudoKey(setKey, state) {
    ...
    if (setKey === 'linkedlist') {
        ...
        const map = { delete: 'deleteHead', reversed: 'reverse', sorted: 'sort', traverse: 'traverse', visit: 'traverse', found: 'search', notFound: 'search' };
        return map[t] || 'traverse';
    }
    if (setKey === 'stack') {
        if (!t) return 'push';
        const map = { insert: 'push', delete: 'pop', found: 'push', notFound: 'push', traverse: 'push' };
        return map[t] || 'push';
    }
    if (setKey === 'queue') {
        if (!t) return 'enqueue';
        const map = { insert: 'enqueue', delete: 'dequeue', found: 'search', notFound: 'search', traverse: 'search' };
        return map[t] || 'enqueue';
    }
    // tree
    if (!t) return 'search';
    const map = { insert: 'insert', found: 'search', notFound: 'search', traverse: 'search', compare: 'search' };
    return map[t] || 'search';
}

// PropTypes for linter satisfaction
Badge.propTypes = { children: PropTypes.node, active: PropTypes.bool, shape: PropTypes.oneOf(['square','circle']), size: PropTypes.number, animate: PropTypes.bool };
Badge.defaultProps = { active: false, shape: 'square', size: 48, animate: false };
LinkedListViz.propTypes = { state: PropTypes.object, list: PropTypes.object, vizSize: PropTypes.number };
LinkedListViz.defaultProps = { vizSize: 56 };
StackViz.propTypes = { state: PropTypes.object, stack: PropTypes.object, vizSize: PropTypes.number };
StackViz.defaultProps = { vizSize: 56 };
QueueViz.propTypes = { state: PropTypes.object, queue: PropTypes.object, vizSize: PropTypes.number };
QueueViz.defaultProps = { vizSize: 56 };
TreeViz.propTypes = { state: PropTypes.object, tree: PropTypes.object, vizSize: PropTypes.number };
TreeViz.defaultProps = { vizSize: 56 };
PseudocodePanel.propTypes = { setKey: PropTypes.string, state: PropTypes.object, map: PropTypes.object };

// Choose which step to show first after executing an operation
function selectInitialStep(op, steps) {
    const nonMutating = new Set(['search', 'traverse', 'inorder', 'preorder', 'postorder']);
...
function runLinkedList({ listRef, setSteps, setCurrentStep, setMessage, title }, op, val, idx) {
    ...
    setSteps(s); setCurrentStep(selectInitialStep(op, s)); setMessage(`${title} - ${op} executed.`);
}

function runStack({ stackRef, setSteps, setCurrentStep, setMessage, title }, op, val) {
...
function runQueue({ queueRef, setSteps, setCurrentStep, setMessage, title }, op, val) {
...
function runTree({ treeRef, setSteps, setCurrentStep, setMessage, title }, op, val) {
